class Admin::OrdersController < AdminController
  ...
  def create
    ...
    if @order.save
      ...
      redirect_to [:admin, edit_order_path(@order)]
    else
  ...
  def edit
    ...
    @order.order_contents.build(:quantity => nil)
    @order.order_contents.build(:quantity => nil)
    @order.order_contents.build(:quantity => nil)
    @order.order_contents.build(:quantity => nil)
    @order.order_contents.build(:quantity => nil)
  ...
  def update
    ...
    array_order_contents.length.times do |i|
      unless array_order_contents[i.to_s]["id"].to_i ==0
    ...
    if @order.update(whitelisted_params)
      ...
      redirect_to [:admin, order_path(@order)]
    else
